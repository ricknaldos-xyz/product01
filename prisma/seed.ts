...
async function main() {
  ...
  const badgeData: { userId: string; badgeType: import('@prisma/client').BadgeType }[] = [
    ...
    { userId: proPlayer.id, badgeType: 'TIER_SEGUNDA' },
    ...
    { userId: elitePlayer.id, badgeType: 'TIER_CUARTA' },
    { userId: elitePlayer.id, badgeType: 'TIER_TERCERA' },
    { userId: elitePlayer.id, badgeType: 'TIER_SEGUNDA' },
    { userId: elitePlayer.id, badgeType: 'DEDICATION_30' },
    { userId: adminUser.id, badgeType: 'FIRST_ANALYSIS' },
    { userId: adminUser.id, badgeType: 'TIER_PRIMERA' },
  ]
  ...
  const tournament = await prisma.tournament.upsert({
    ...
    create: {
      ...
      description: 'Torneo abierto de tenis para jugadores de 2da B en adelante. 16 participantes.',
  ...
  const notifications = [
    ...
    { userId: elitePlayer.id, type: 'TIER_PROMOTION' as const, title: 'Ascenso de categoria!', body: 'Has subido a la categoria 2da A' },

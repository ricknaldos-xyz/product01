...
import { ArrowLeft, Loader2, User, MapPin, Wrench, Save, ChevronRight } from 'lucide-react'
...
/**
 * Valid next statuses the provider can transition to from the current status.
 * This mirrors the STRINGING_ORDER_TRANSITIONS from lib/order-transitions.ts
 * but only includes transitions relevant to the provider workflow.
 */
const PROVIDER_TRANSITIONS: Record<string, { status: string; label: string; variant: 'primary' | 'solid' | 'outline' | 'destructive' }[]> = {
  CONFIRMED: [
    { status: 'PICKUP_SCHEDULED', label: 'Programar recojo', variant: 'primary' },
    { status: 'STRINGING_CANCELLED', label: 'Cancelar', variant: 'destructive' },
  ],
  PICKUP_SCHEDULED: [
    { status: 'RECEIVED_AT_WORKSHOP', label: 'Marcar como recibido', variant: 'primary' },
    { status: 'STRINGING_CANCELLED', label: 'Cancelar', variant: 'destructive' },
  ],
  RECEIVED_AT_WORKSHOP: [
    { status: 'IN_PROGRESS', label: 'Iniciar encordado', variant: 'outline' },
    { status: 'STRINGING_CANCELLED', label: 'Cancelar', variant: 'destructive' },
  ],
  IN_PROGRESS: [
    { status: 'STRINGING_COMPLETED', label: 'Encordado completado', variant: 'solid' },
  ],
  STRINGING_COMPLETED: [
    { status: 'READY_FOR_PICKUP', label: 'Listo para recoger', variant: 'solid' },
    { status: 'OUT_FOR_DELIVERY', label: 'Enviar a domicilio', variant: 'primary' },
  ],
  READY_FOR_PICKUP: [
    { status: 'DELIVERED', label: 'Marcar como entregado', variant: 'solid' },
  ],
  OUT_FOR_DELIVERY: [
    { status: 'DELIVERED', label: 'Marcar como entregado', variant: 'solid' },
  ],
...
const TIMELINE_STEPS = [
  ...
  { key: 'confirmedAt', label: 'Confirmado' },
  { key: 'receivedAt', label: 'Recibido en taller' },
  { key: 'completedAt', label: 'Encordado completado' },
  { key: 'deliveredAt', label: 'Entregado' },
  { key: 'cancelledAt', label: 'Cancelado' },
]
...
export default function ProviderOrderDetailPage() {
  ...
  const fetchOrder = useCallback(async () => {
    try {
      const res = await fetch(`/api/provider/workshops/${id}/orders/${orderId}`)
      if (res.ok) {
        ...
        router.push(`/provider/workshops/${id}/orders`)
      }
    } catch {
  ...
  const transitions = PROVIDER_TRANSITIONS[order.status] || []

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <GlassButton variant="ghost" size="sm" onClick={() => router.push(`/provider/workshops/${id}/orders`)}>
            ...
            <div className="mt-4 pt-4 border-t border-glass">
              ...
              <div className="space-y-3">
                {TIMELINE_STEPS.map((step) => {
                  const value = order[step.key as keyof StringingOrderDetail] as string | null
                  if (!value) return null
                  return (
                    <div key={step.key} className="flex items-center gap-3">
                      <div className={`w-2.5 h-2.5 rounded-full ${step.key === 'cancelledAt' ? 'bg-destructive' : 'bg-primary'}`} />
        ...
        <div className="space-y-6">
          ...
          <GlassCard intensity="light">
            ...
            <div className="space-y-2 text-sm">
              ...
              {!order.preferredPickupDate && !order.scheduledPickupDate && !order.estimatedCompletionDate && (
                <p className="text-muted-foreground">Sin fechas programadas</p>
              )}
            </div>
          </GlassCard>
        </div>
